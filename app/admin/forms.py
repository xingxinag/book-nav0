...
class SiteSettingsForm(FlaskForm):
    ...
    # WebDAV备份设置
    webdav_enabled = BooleanField('启用WebDAV备份')
    webdav_url = StringField('WebDAV服务器地址', validators=[Optional(), Length(max=512)],
                           description='支持HTTP和HTTPS协议，例如: https://example.com/webdav')
    webdav_username = StringField('用户名', validators=[Optional(), Length(max=128)])
    webdav_password = PasswordField('密码', validators=[Optional(), Length(max=256)])
    webdav_path = StringField('远程路径', validators=[Optional(), Length(max=256)],
                             default='/backups/', description='WebDAV服务器上的备份目录路径')
    webdav_auto_backup = BooleanField('自动备份到WebDAV', description='创建本地备份时自动上传到WebDAV')
    webdav_keep_local = BooleanField('保留本地备份', default=True,
                                   description='上传到WebDAV后是否保留本地备份文件')
