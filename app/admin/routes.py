...
from app.utils.webdav_backup import backup_to_webdav, create_webdav_client
import time
import json
import threading
...
def site_settings():
    try:
        ...
        if form.validate_on_submit():
            ...
            # 更新WebDAV备份设置
            settings.webdav_enabled = form.webdav_enabled.data
            settings.webdav_url = form.webdav_url.data
            settings.webdav_username = form.webdav_username.data
            # 如果密码字段不为空，则编码存储
            if form.webdav_password.data:
                from app.utils.webdav_backup import encrypt_password
                settings.webdav_password = encrypt_password(form.webdav_password.data)
            settings.webdav_path = form.webdav_path.data or '/backups/'
            settings.webdav_auto_backup = form.webdav_auto_backup.data
            settings.webdav_keep_local = form.webdav_keep_local.data
...
def backup_data():
    ...
    try:
        ...
        if settings.webdav_enabled and settings.webdav_auto_backup:
            webdav_success, webdav_msg = backup_to_webdav(backup_path, settings)
            if webdav_success:
                webdav_message = f" (WebDAV备份成功: {webdav_msg})"
                current_app.logger.info(f"WebDAV自动备份成功: {filename}")
            else:
                webdav_message = f" (WebDAV备份失败: {webdav_msg})"
                current_app.logger.warning(f"WebDAV自动备份失败: {webdav_msg}")

        flash(f'数据库备份成功{webdav_message}', 'success')
...
@bp.route("/test-webdav", methods=["POST"])
@login_required
@superadmin_required
def test_webdav():
    """测试WebDAV连接"""
    try:
        ...
        if not webdav_client:
            return jsonify({
                "success": False,
                "message": "WebDAV配置无效或未启用"
            })

        success, message = webdav_client.test_connection()
        return jsonify({
            "success": success,
            "message": message
        })
    except Exception as e:
        current_app.logger.error(f"WebDAV连接测试失败: {str(e)}")
        return jsonify({
            "success": False,
...
@bp.route("/backup-to-webdav/<filename>", methods=["POST"])
@login_required
@superadmin_required
def backup_file_to_webdav(filename):
    """手动将指定备份文件上传到WebDAV"""
    # 安全检查
    if os.path.sep in filename or ".." in filename:
        return jsonify({
    ...
    try:
        ...
        settings = SiteSettings.get_settings()
        success, message = backup_to_webdav(backup_path, settings)

        if success:
            current_app.logger.info(f"手动WebDAV备份成功: {filename}")
        else:
            current_app.logger.error(f"手动WebDAV备份失败: {filename} - {message}")
        ...
        current_app.logger.error(f"WebDAV备份失败: {str(e)}")
        return jsonify({
            "success": False,
...
@bp.route("/backup-all-to-webdav", methods=["POST"])
@login_required
@superadmin_required
def backup_all_to_webdav():
    """批量将本地备份文件上传到WebDAV"""
    try:
        settings = SiteSettings.get_settings()
        ...
        backup_files = [f for f in os.listdir(backup_dir) if f.endswith(".db3")]
        if not backup_files:
            return jsonify({
                "success": False,
                "message": "没有找到备份文件"
        ...
        for filename in backup_files:
            ...
            if success:
                success_count += 1
                current_app.logger.info(f"批量WebDAV备份成功: {filename}")
            else:
                failed_count += 1
                current_app.logger.error(f"批量WebDAV备份失败: {filename} - {message}")

        return jsonify({
            "success": True,
            "message": f"批量备份完成: 成功 {success_count} 个, 失败 {failed_count} 个"
        })
    except Exception as e:
        current_app.logger.error(f"批量WebDAV备份失败: {str(e)}")
        return jsonify({
            "success": False,
            "message": f"批量备份失败: {str(e)}"
        })
