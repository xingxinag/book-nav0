...
class SiteSettings(db.Model):
    ...
    # WebDAV备份设置
    webdav_enabled = db.Column(db.Boolean, default=False)  # 是否启用WebDAV备份
    webdav_url = db.Column(db.String(512), nullable=True)  # WebDAV服务器地址
    webdav_username = db.Column(db.String(128), nullable=True)  # WebDAV用户名
    webdav_password = db.Column(db.String(256), nullable=True)  # WebDAV密码(加密存储)
    webdav_path = db.Column(db.String(256), default='/backups/')  # WebDAV备份路径
    webdav_auto_backup = db.Column(db.Boolean, default=False)  # 是否自动备份到WebDAV
    webdav_keep_local = db.Column(db.Boolean, default=True)  # 是否保留本地备份
